...
public class DeckShopGenerator : MonoBehaviour
{
    ...
    [SerializeField] GameObject rerollSound;
    [SerializeField] GameObject hoverSound;
    [SerializeField] GameObject buySound;
    ...
    public void AttemptBuy(GameObject pressedObj)
    {
        ...
        reloadCostText.text = DominoScore.instance.FormatLargeNumber(System.Math.Floor(Mathf.Pow(1.8f, lifetimePurchases) * 2));
    ...
    {        
        ...
        double generateCost = System.Math.Floor(Mathf.Pow(1.8f, lifetimePurchases) * 5);

        if (!ignoreCost)
        {
            if (DominoScore.instance.score < generateCost)
                return;

            DominoScore.instance.score -= generateCost;
        }

        if (!ignoreCost)
            lifetimeReloads += 1;
        
        reloadCostText.text = DominoScore.instance.FormatLargeNumber(generateCost);
